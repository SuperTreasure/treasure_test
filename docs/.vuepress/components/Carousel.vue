...
import { rafTimeout, cancelRaf, requestAnimationFrame, cancelAnimationFrame } from './utils'
...
<template>
    <div class="m-slider" ref="carousel"
        ...
        <div :class="{ 'transition': transition }"
            :style="`width: ${totalWidth}px; height: 100%; will-change: transform; transform: translateX(${-left}px);`">
            <div class="m-image" v-for="(image, index) in images" :key="index">
                <Spin :spinning="!complete[ index ]" indicator="dynamic-circle">
                    <a :href="image.link ? image.link : 'javascript:;'" :target="image.link ? '_blank' : '_self'"
                        class="m-link">
                        <img @load="onComplete( index )" :src="image.src" :key="image.src" :alt="image.title"
                            class="u-img" :style="`width: ${imageWidth}px; height: ${imageHeight}px;`" />
                    </a>
                </Spin>
            </div>
            <div class="m-image" v-if="imageCount">
                <Spin :spinning="!complete[ 0 ]" indicator="dynamic-circle">
                    <a :href="images[ 0 ].link ? images[ 0 ].link : 'javascript:;'"
                        :target="images[ 0 ].link ? '_blank' : '_self'" class="m-link">
                        <img @load="onComplete( 0 )" :src="images[ 0 ].src" :key="images[ 0 ].src"
                            :alt="images[ 0 ].title" class="u-img"
        ...
        <template v-if="navigation">
            <svg class="arrow-left" :style="`width: ${navSize}px; height: ${navSize}px;`" @click="onLeftArrow"
            ...
            <svg class="arrow-right" :style="`width: ${navSize}px; height: ${navSize}px;`" @click="onRightArrow"
                xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
                <path
        ...
        <div class="m-switch" v-if="pagination">
            <div @click="onSwitch( n )" :class="[ 'u-circle', { 'active': activeSwitcher === n } ]"
                :style="[ { width: `${pageSize}px`, height: `${pageSize}px` }, pageStyle ]" v-for="n in imageCount"
                :key="n">
            </div>
        </div>
    </div>
